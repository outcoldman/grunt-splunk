...
var SplunkService = function(grunt) {
  ...
  var _splunkExec = function(args, done) {
    if (splunkHome) {
      ...
      grunt.log.debug('Splunk: launching command "' + args.join(args) + '"');
      var splunkCommand = path.join(splunkHome, 'bin',  (process.platform === 'win32') ? 'splunk.exe' : 'splunk');
      grunt.log.debug('Splunk: command is "' + splunkCommand + '"');
      var splunkProc = child_process.spawn(splunkCommand, args, { stdio: 'inherit' }, function(err) {
        if (err) {
          grunt.log.error(err);
          grunt.warn('Splunk: command ' + args.join(args) + ' failed.');
        } else {
          grunt.log.ok('Splunk: command "' + args.join(args) + '" is successfully completed.');
        }
        done();
      });
    } else {
    ...
    async.waterfall([
      ...
      function(service, next) {
        ...
        service.get('/services/apps/local/_reload', {}, function(err, response){
          next(err, service, response);
        });
      },
      // Load messages
      function(service, response, next) {
        grunt.log.debug('Splunk: \n' + JSON.stringify(response, null, '\t'));
        grunt.log.debug('Splunk: requesting /services/messages.');
        service.get('/services/messages', {}, next);
      },
